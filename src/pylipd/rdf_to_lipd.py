...
class RDFToLiPD(object):
    ...
    def get_property_details(self, pname, schema) :
        details = { "name": pname }
        # Create property
    ...
    def get_rdf_property_details(self, pname, fullkey, schema) :
        key = pname
        pname = lcfirst(pname)
        details = { "name": pname }
    ...
    def get_schema_reverse_map(self) :
        ...
        for schid,sch in SCHEMA.items() :
            ...
            for prop,details in sch.items() :
                ...
                pdetails = self.get_property_details(prop, sch)
                pname = pdetails["name"]
                pdetails["name"] = prop
    ...
    def get_indexed_facts(self, id):
        if id in self.allfacts:
            return

        facts = self.get_facts(id)
        ...
        for pname, pfacts in facts.items():
            for pfact in pfacts:
                if pfact["@type"] == "uri":
                    if pname != "type":
                        self.get_indexed_facts(pfact["@id"])


    def convert_to_lipd(self, id, category, schemaname, pagesdone=[]) :
        if id in self.allfacts:
            ...
            if "type" in facts:
                ...
                for cat in cats:
                    if (cat["@type"] == "uri") :
                        category = self.local_name(cat["@id"])
                        break
                        ...
                        if value["@id"] in self.allfacts :
                            ...
                            if "type" in pfact:
                                ...
                                for valcat in valcats:
                                    if (valcat["@type"] == "uri") :
                                        valcatname = self.local_name(valcat["@id"])
                                        propkey = prop + "." + valcatname
                                        break

                details = self.get_rdf_property_details(prop, propkey, schema)
                name = details["name"]
    ...
    def post_processing(self, obj, parent=None):
        ...
        for key,value in obj.items():
            if type(value) is list:
                for i in range(len(value)):
                    obj[key][i] = self.post_processing(value[i], obj)
            else:
    ...
    # Converters
    #########################################
    def location_to_json(self, geo, parent = None) :
        geojson = {
            "geometry":
                {
                    "coordinates" : [0, 0, 0],
    ...
    def get_google_spreadsheet_key(self, url:str, parent = None) :
        return url.replace("https://docs.google.com/spreadsheets/d/", "")

    def remove_found_in_table(self, var, parent = None) :
        ...
        if type(item) is dict:
            ...
            for key,value in item.items() :
                if (key == "archiveType") :
                    ...
                    [nitem[key], atypes] = self._get_variable_archive_types(value, atypes)
            
        elif type(item) is list:
            nitem = []
            for value in item :
                [nit, atypes] = self._get_variable_archive_types(value, atypes)
                nitem.append(nit)
    ...
    def extract_from_proxy_system(self, var, parent = None) :
    ...
    def unwrap_uncertainty(self, var, parent = None) :
    ...
    def unwrap_integration_time(self, interp, parent = None) :
    ...
    def collect_variables_by_id(self, item, arr) :
        ...
        if ("@category" in item) and ("@id" in item) and re.match("/Variable\$/", item["@category"]) :
            ...
            for key,value in item.items() :
                if (key[0] != "@") :
                    arr = self.collect_variables_by_id(item[key], arr)
        return arr

    def set_variable_type(self, var, parent = None) :
    ...
    def remove_depth_property(self, val, parent = None) :
        if (("takenAtDepth" in val)) :
            del val["takenAtDepth"]
        return val

    def create_publication_identifier(self, pub, parent = None) :
        identifiers = []
        if (("hasDOI" in pub)) :
            identifier = {}
            identifier["type"] = "doi"
    ...
    def change_seasonality_type(self, interp, parent = None) :
    ...
    def values_to_array(self, resolution, parent = None) :
